...
//------------------------------------------------------------------------------------------------------------------
// // Board class fully qualified constructor with moveNumber as parameter
// Board::Board( int theBoardNumber, int theBoard[ BoardRows * BoardColumns], int theParValue,int themoveNumber)
// {
//     // Set the board number
//     boardNumber = theBoardNumber;
...
// } // end Board::Board(..., moveNumber)


//------------------------------------------------------------------------------------------------------------------
// Board class copy constructor
Board::B```rd( const Board &existingBoard)
{
...
//-------------------------------------------------------------------------------------
//Constructor
// Node::Node(Board theOldBoard, int thescore, int theboardIndex,Node* pNext)
// {
//   oldBoard=theOldBoard;
//   score=thescore;
//   boardIndex=theboardIndex;
//   //pNext = NULL;
// }
...
void displayList( Node *pTemp, int theScore)
{
     int score=theScore;
           cout<<"   Score: "<<score<<", List of board:moveNumber is: ";
    while( pTemp!= NULL) {
        if(pTemp->oldBoard.getMoveNumber()==0 && pTemp->currentBoardIndex==1){
        cout<<pTemp->currentBoardIndex<<":"<<pTemp->oldBoard.getMoveNumber();
        }
       else{
          cout<<pTemp->currentBoardIndex<<":"<<pTemp->oldBoard.getMoveNumber()<<"->";
       }
...
void deleteNodeFromList(Node* &pHead,Board &theOldBoard, int &theScore, int &theBoardIndex) 
{
   Node *pTemp;
   ...
   theOldBoard=pHead->oldBoard;
   theScore=pHead->score;
   theBoardIndex=pHead->currentBoardIndex;
...
//    the score, which should be restored from the list
//    the currentBoardIndex, which should be restored from the list
void addNodeToList(Node* &pHead,Board &theOldBoard, int theScore, int theBoardIndex)
{
    //Create a new node and store current values into it
    Node* newNode= new Node;// (theBoard,score);
    newNode->oldBoard = theOldBoard;
    newNode->score = theScore;
    newNode->currentBoardIndex = theBoardIndex;
    newNode->pNext = pHead;
    pHead = newNode;
    ...
    while( true) {
      ...
      // Make moves to try and solve the puzzle of the current level
        while( ! theBoard.boardIsClear() ) {
            ...
            // ...
            displayList( pHead, score);
            //addNodeToList(pHead,theBoard,score,currentBoardIndex);
            //cout<<"      Score: "<<score<<", List of board:moveNumber is: "<<currentBoardIndex<<":"<<score << endl;
